...
function statRow(label, value, x, y, accent) {
  return `
  <rect x="${x}" y="${y}" width="6" height="14" rx="1" fill="${accent}"/>
  <text x="${x + 14}" y="${y + 11}" font-family="'JetBrains Mono', 'Fira Mono', monospace" font-size="11" fill="#8b8b8b">${label}</text>
  <text x="${x + 180}" y="${y + 11}" font-family="'JetBrains Mono', 'Fira Mono', monospace" font-size="11" fill="#e0e0e0" text-anchor="end">${value}</text>`;
}
...
export default async function handler(req, res) {
  ...
  ${rows.map(([label, value, i]) => {
    ...
    return statRow(label, value, padX, padY + i * rowH, accent);
  }).join('')}
