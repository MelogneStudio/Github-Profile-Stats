...
export default async function handler(req, res) {
  ...
  try {
    const counts = await fetchHeatmap(username);
    ...
    for (let i = 0; i < DAYS; i++) {
      ...
      cells.push(`<rect x="${x}" y="${y}" width="${CELL}" height="${CELL}" rx="2" fill="${heatColor(count)}"><title>${key}: ${count}</title></rect>`);
    }
    ...
    const dayLabels = [1, 3, 5].map(i =>
      `<text x="${padX + DAY_LABELS_W - 4}" y="${padY + i * STEP + 9}" font-family="'JetBrains Mono','Fira Mono',monospace" font-size="9" fill="#444" text-anchor="end">${dayNames[i]}</text>`
    );

    // legend
    const legendColors = ['#222222', '#1a4a45', '#1e7a6e', '#2ec4b6', '#7eeee8'];
    const legendX = W - padX - legendColors.length * (CELL + 2);
    const legendY = H - 18;
  ...
  ${legendCells.join('')}
  <text x="${legendX + legendColors.length * (CELL + 2) + 4}" y="${legendY + 9}" font-family="'JetBrains Mono','Fira Mono',monospace" font-size="9" fill="#444">more</text>
</svg>`;
