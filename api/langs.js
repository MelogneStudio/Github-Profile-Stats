const LANG_COLORS = {
  JavaScript:'#f4a261',TypeScript:'#2ec4b6',Python:'#3572A5',Java:'#b07219',
  'C++':'#f34b7d',C:'#8b8b8b','C#':'#178600',Go:'#2ec4b6',Rust:'#f4a261',
  Ruby:'#701516',PHP:'#7a86b8',Swift:'#F05138',Kotlin:'#c77dff',HTML:'#e34c26',
  CSS:'#7a86b8',Shell:'#89e051',Dart:'#2ec4b6',Vue:'#41b883',Lua:'#f4a261',
  Haskell:'#c77dff',Elixir:'#c77dff',Zig:'#f4a261',
};
function lc(l) { return LANG_COLORS[l] || '#555'; }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const r = await fetch(`https://api.github.com/users/${username}/repos?sort=pushed&per_page=50&type=owner`, { headers });
    ...
    const langs = sorted.map(([lang, count]) => ({ lang, pct: Math.round(count / total * 100) }));

    const padX = 20, padY = 52, barH = 8, rowH = 26;
    const W = 320, H = padY + barH + 12 + langs.length * rowH + 20;

    let barX = padX;
    const barW = W - padX * 2;
    const segments = langs.map(({ lang, pct }) => {
      const w = Math.round(barW * pct / 100);
      const seg = `<rect x="${barX}" y="${padY}" width="${w}" height="${barH}" fill="${lc(lang)}" opacity="0.9"/>`;
      barX += w;
      return seg;
    });

    const rowsSVG = langs.map(({ lang, pct }, i) => {
      const y = padY + barH + 14 + i * rowH;
      return `
  <rect x="${padX}" y="${y + 2}" width="8" height="8" rx="2" fill="${lc(lang)}"/>
  ...
  <rect x="${padX}" y="${padY}" width="${barW}" height="${barH}" rx="2" fill="#2a2a2a"/>
  ${segments.join('')}
  ${rowsSVG}
